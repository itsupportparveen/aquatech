...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'site2'),
        'collapse' => __('collapse child menu', 'site2'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'site2'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'site2'),
    ...
    foreach ($widgets as $widget) {
        if (false !== strpos($widget['content'], '{catItemClass}')) {
            ...
            if ($exportIconHtml !== '{icon_html}') {
                ...
                $widget['content'] = preg_replace('/{catLinkClass}/', 'u-button-style u-category-link u-nav-link ', $widget['content']);
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'site2');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'site2'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'site2'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'site2'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'site2'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'site2'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'site2')
    ));
    extract($args);
...
function get_categories_filter_html($args) {
    ...
    // add item all
    $categories_filter_html .= strtr($args['itemTemplate'], array('{categories_filters_content}' => __('All', 'site2'), '{categories_filters_value}' => get_permalink(wc_get_page_id('shop'))));
    // add item featured
    $featuredLink = class_exists('Woocommerce') ? add_query_arg('featured', 'true', get_permalink(wc_get_page_id('shop'))) : '';
    $item = strtr($args['itemTemplate'], array('{categories_filters_content}' => __('Featured', 'site2'), '{categories_filters_value}' => $featuredLink));
...
function generate_category_options( $categories, $itemTemplate, $parent = 0, $prefix = '' ) {
    ...
    foreach ($categories as $category) {
        if ($category->parent == $parent) {
            ...
            if (get_term_link(get_queried_object_id(), 'product_cat') === $link) {
                $item = strtr($item, array('<option' => '<option selected="selected"'));
            }
            $result .= $item;
